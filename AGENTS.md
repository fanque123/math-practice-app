...
- **APK 发布**：Releases 有 `v1.0`，直链 **https://github.com/fanque123/math-practice-app/releases/download/v1.0/math-practice-v1.0.apk**（asset 用 ASCII 文件名避免 URL 编码问题）。发新版：本地构建新 APK → 升 versionCode/versionName（`android/app/build.gradle`）→ API 或网页上传新 asset。
