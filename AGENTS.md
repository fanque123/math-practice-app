...
service-worker.js   离线缓存（**网络优先**：先 fetch 最新代码并写缓存，断网才回退缓存——避免旧缓存导致"改了不生效"），缓存名 math-practice-v19（改任何前端文件都要升这个版本号）
...
- **原生识别的错误补发**：APK 里语音答对一题后，停止识别会让 `SpeechRecognizer` 补发 `ERROR_CLIENT`，若不拦截会把正确反馈覆盖成「语音识别出错」。双保险：app.js 的 `recognition.onerror` 里 `state.isAnswering` 为真就直接忽略；原生端 `resultsDelivered` 标记拿到结果后不再上报错误。同理，WebView 没有 `speechSynthesis` 对象，任何语音停止调用都必须走 `stopAllSpeech()`（原桥/浏览器双路径），直接调 `window.speechSynthesis.cancel()` 会在 APK 里抛异常——做题页旧的 ✕ 按钮就是这样失灵的。退出按钮：`#quiz-back-btn`（回设置页）、`#quiz-exit-btn` 和 `#setup-exit-btn`（全端显示，APK 里调 `AndroidNative.exit()` 关应用，浏览器里等于回首页）。
