...
app.js              全部逻辑（1259 行，单 IIFE，下方详解）
...
- **APK 发布**：Releases 有 `v1.0`，直链 **https://github.com/fanque123/math-practice-app/releases/latest/download/math-practice.apk**（latest 永远指向最新 Release 的同名 asset，发新版只需替换 asset，链接不变）。发新版：本地构建新 APK → 升 versionCode/versionName（`android/app/build.gradle`）→ 删旧 asset 上传同名新 asset。
